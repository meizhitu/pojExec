...
void printArr(int deep,int l,int r){
	printf("deep=%d\n",deep);
	for(int i =l;i<=r;i++)
		printf("%d,",toleft[deep][i]);
	printf("\n");
}
...
int query(int left, int right, int k, int L, int R, int deep)
{
	...
	int mid = (L + R) >> 1;
	int x = toleft[deep][left - 1] - toleft[deep][L - 1];//位于left左边的,放于左子树中的数字个数
	int y = toleft[deep][right] - toleft[deep][L - 1];//到right为止位于左子树的个数
	int ry = right - L - y;//到right左边为止位于右子树的数字个数
